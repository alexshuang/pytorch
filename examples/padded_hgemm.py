...
# NN
a = torch.randn(500, 300).half().cuda()
b = torch.randn(300, 1000).half().cuda()
...
cc = aa @ bb
print("NN: c == cc: {}".format(torch.allclose(c.float().cpu(), cc.float().cpu())))

# TN
a = torch.randn(300, 500).half().cuda()
b = torch.randn(300, 1000).half().cuda()
aa, bb = a.clone().detach(), b.clone().detach()
a = F.pad(a, (0, 32)).narrow(-1, 0, a.size(1))
b = F.pad(b, (0, 32)).narrow(-1, 0, b.size(1))
...
cc = aa.T @ bb
print("TN: c == cc: {}".format(torch.allclose(c.float().cpu(), cc.float().cpu())))

# NT
a = torch.randn(500, 300).half().cuda()
b = torch.randn(1000, 300).half().cuda()
aa, bb = a.clone().detach(), b.clone().detach()
a = F.pad(a, (0, 32)).narrow(-1, 0, a.size(1))
b = F.pad(b, (0, 32)).narrow(-1, 0, b.size(1))
print("A size: {}, A stride: {}".format(a.size(), a.stride()))
...
cc = aa @ bb.T
print("NT: c == cc: {}".format(torch.allclose(c.float().cpu(), cc.float().cpu())))
...
cc = aa.T @ bb.T
print("TT: c == cc: {}".format(torch.allclose(c.float().cpu(), cc.float().cpu())))
